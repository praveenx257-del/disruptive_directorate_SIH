...
function StatCard({ icon: Icon, label, value, subtitle, accent = "blue" }) {
  const accents = {
        ...
        <div className="space-y-6 p-6">
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`mt-2 text-4xl font-bold ${riskStyles.text}`}>
                  {Number(district.average_risk ?? 0).toFixed(1)}
                </p>

                <p className="mt-2 text-sm text-slate-500">
                  Maximum observed risk:{" "}
                  <span className="text-slate-300">
                    {Number(district.maximum_risk ?? 0).toFixed(1)}
                  </span>
                </p>
              </div>

              <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-slate-800 bg-[#080f20]">
                <ShieldAlert className={riskStyles.text} size={26} />
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-xl border border-slate-800 bg-[#0b1224] p-4">
                ...
                <p className="mt-2 text-xl font-semibold text-white">
                  {formatNumber(district.constituency_count)}
                </p>
              </div>

              <div className="rounded-xl border border-red-500/10 bg-red-500/5 p-4">
                <p className="text-xs text-slate-500">Anomalous Works</p>
              ...
              <div className="rounded-xl border border-red-500/10 bg-red-500/5 p-4">
                <p className="text-xs text-slate-500">High Risk Works</p>
                <p className="mt-2 text-xl font-semibold text-red-400">
              ...
              <div className="rounded-xl border border-amber-500/10 bg-amber-500/5 p-4">
                <p className="text-xs text-slate-500">Medium Risk Works</p>
                <p className="mt-2 text-xl font-semibold text-amber-400">
                  {formatNumber(district.medium_risk_works)}
                </p>
              </div>

              <div className="rounded-xl border border-slate-800 bg-[#0b1224] p-4">
                <p className="text-xs text-slate-500">Recommended Amount</p>
                <p className="mt-2 text-xl font-semibold text-white">
                  {formatCurrency(district.total_recommended_amount)}
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-2 text-2xl font-bold text-white">
                    {formatCurrency(district.total_expenditure)}
                  </p>
                </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <AlertMini
                label="Near ₹5L"
                value={district.alerts?.near_5l_threshold}
                icon={Wallet}
              />

              <AlertMini
                label="Negative Timeline"
                value={district.alerts?.negative_timeline}
                icon={AlertTriangle}
              />

              <AlertMini
                label="Vague Description"
                value={district.alerts?.vague_description}
                icon={Search}
              />
            </div>
          </div>
  ...
  const totals = useMemo(() => {
    ...
    return {
      ...
      anomalies: districts.reduce(
        (sum, item) => sum + Number(item.anomalous_works ?? 0),
        0
      ),
      highRisk: districts.reduce(
        (sum, item) => sum + Number(item.high_risk_works ?? 0),
        0
      ),
    };
  }, [districts]);
      ...
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-slate-500">
            Compare risk patterns across Implementing District Authorities
            and identify areas with concentrated audit signals.
          </p>
        </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-800 bg-[#0b1224]">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              {loading
                ...
                : `${formatNumber(filteredDistricts.length)} districts displayed`}
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="px-6 py-16 text-center">
            <Map className="mx-auto text-slate-700" size={32} />
                      ...
                      >
                        ...
                        <td className="px-6 py-5">
                          <div className="flex justify-end gap-2">
                            {Number(
                              district.alerts?.ghost_asset ?? 0
                            ) > 0 && (
                              <span
                                title="Ghost asset alerts"
                                className="rounded-lg border border-red-500/20 bg-red-500/10 px-2 py-1 text-[10px] font-semibold text-red-400"
                              >
                                GHOST
                              </span>
                            )}
                            ...
                            {Number(
                              district.alerts?.batch_split ?? 0
                            ) > 0 && (
                              <span
                                title="Batch split alerts"
                                className="rounded-lg border border-amber-500/20 bg-amber-500/10 px-2 py-1 text-[10px] font-semibold text-amber-400"
                              >
                                SPLIT
                              </span>
                            )}
                            ...
                            {Number(
                              district.alerts?.near_5l_threshold ?? 0
                            ) > 0 && (
                              <span
                                title="Near ₹5 lakh alerts"
                                className="rounded-lg border border-blue-500/20 bg-blue-500/10 px-2 py-1 text-[10px] font-semibold text-blue-400"
                              >
                                ₹5L
                              </span>
                            )}
